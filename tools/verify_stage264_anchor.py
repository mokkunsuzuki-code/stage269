...
def main() -> None:
    ...
    for artifact in artifacts:
        ...
        if not artifact_path.exists():
            print(f"[ERROR] missing artifact referenced by manifest: {artifact_path}")
            sys.exit(1)
        actual_sha = sha256_file(artifact_path)
        if actual_sha != expected_sha:
            print(f"[ERROR] artifact sha256 mismatch: {artifact_path}")
            print(f"        expected: {expected_sha}")
            print(f"        actual:   {actual_sha}")
            sys.exit(1)
        print(f"[OK] artifact verified: {artifact_path}")
